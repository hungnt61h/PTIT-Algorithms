#include <iostream>
#include <stdio.h>
#include <stdlib.h>

using namespace std;

typedef struct Queue {
    int front;
    int rear;
    int capacity;
    int* array;
} Queue;
 

Queue* createQueue(long capacity) {
    Queue* queue = (Queue*) malloc(sizeof(Queue));
    queue->capacity = capacity;
    queue->front = queue->rear = -1;
    queue->array = (int*) malloc (queue->capacity*sizeof(int));
    return queue;
}

long isEmpty (Queue* queue) {
    return queue->front == -1;
}

void Enqueue (Queue* queue, int item) {
    queue->array[++queue->rear] = item;
    if (isEmpty(queue))
        ++queue->front;
}

int Dequeue (Queue* queue) {
    long item = queue->array[ queue->front ];
    if( queue->front == queue->rear )
        queue->front = queue->rear = -1;
    else
        queue->front++;
    return item;
}

int compareAsc(const void* a, const void* b) {
    return *(int*)a > *(int*)b;
}

int compareDesc(const void* a, const void* b) {
    return *(int*)a < *(int*)b;
}
 

void populateAux (int* aux, Queue* queue0, Queue* queue1, Queue* queue2, long* top) {
    while (!isEmpty(queue0))
        aux[(*top)++] = Dequeue(queue0);
    while (!isEmpty(queue1))
        aux[(*top)++] = Dequeue(queue1);
    while (!isEmpty(queue2))
        aux[(*top)++] = Dequeue(queue2);
}

void printArr (int* arr, long size) {
    for (long i = 0; i < size; ++i)
        cout<<arr[i];
}

long findMaxMultupleOf3(int* arr, long size) {
    qsort(arr, size, sizeof(int), compareAsc);
    Queue* queue0 = createQueue(size);
    Queue* queue1 = createQueue(size);
    Queue* queue2 = createQueue(size);
    long i, sum;
    for (i = 0, sum = 0; i < size; i++) {
        sum += arr[i];
        if (arr[i]%3 == 0)
            Enqueue(queue0, arr[i]);
        else if (arr[i]%3 == 1)
            Enqueue(queue1, arr[i]);
        else
            Enqueue(queue2, arr[i]);
    }
    if (sum%3 == 1) {
        if (!isEmpty(queue1))
            Dequeue(queue1);
        else {
            if (!isEmpty(queue2))
                Dequeue(queue2);
            else
                return 0;
            if (!isEmpty(queue2))
                Dequeue(queue2);
            else
                return 0;
        }
    }
    else if (sum%3 == 2) {
        if (!isEmpty(queue2))
            Dequeue(queue2);
        else {
            if (!isEmpty(queue1))
                Dequeue(queue1);
            else
                return 0;
            if (!isEmpty(queue1))
                Dequeue(queue1);
            else
                return 0;
        }
    }
 
    int aux[size];
	long top = 0;
    populateAux (aux, queue0, queue1, queue2, &top);
    qsort (aux, top, sizeof( int ), compareDesc);
    if (top != 0)
 		printArr(aux, top);
	else
		cout<<"-1";
	return top;
}

void test() {
	long n;
	cin>>n;
	int arr[n];
	for (long i = 0; i < n; i++)
		cin>>arr[i];
	findMaxMultupleOf3(arr, n);
}

int main() {
	int t;
	cin>>t;
	while (t--) {
		test();
		if (t > 0)
			cout<<endl;
	}
    return 0;
}
